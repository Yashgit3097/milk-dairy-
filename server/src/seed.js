...
async function seed() {
  ...
  if (!customer) {
    console.error('FAIL: Verification Customer not found. Make sure database is running and verify_phase1.js has been run.');
    process.exit(1);
  }
  ...
  const julyDays = {};
  let julyTotalMl = 0;
  // Fill July: 500ml on odd days, 1000ml on even days, skip on multiples of 5
  for (let d = 1; d <= 31; d++) {
    if (d % 5 === 0) {
      julyDays[String(d)] = 0; // 0 means no delivery
    } else {
  ...
  // 2. Seed August 2026 Entries (up to day 20)
  const augustDays = {};
  let augustTotalMl = 0;
  for (let d = 1; d <= 20; d++) {
    if (d % 4 === 0) {
